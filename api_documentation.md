...
**Версия API:** v1 (планируется: `/api/ai/v1/`)  
**Текущая версия:** 1.0.0

> **Примечание:** Версионирование API будет добавлено в ближайшем релизе. Все текущие endpoints будут доступны по `/api/ai/v1/` для обеспечения обратной совместимости.
...
API будет использовать версионирование для обеспечения обратной совместимости:

- **Текущая версия:** v1 (в разработке)
- **Формат URL:** `https://xbaza.by/api/ai/v1/{endpoint}`
- **Заголовок версии:** `X-API-Version: 1.0` (в ответах)

**Пример:**
```bash
# Текущий формат (будет поддерживаться)
curl -H "User-Agent: ChatGPT-User" \
  "https://xbaza.by/api/ai/jobs"

# Будущий формат с версионированием
curl -H "User-Agent: ChatGPT-User" \
  "https://xbaza.by/api/ai/v1/jobs"
```

**Политика версионирования:**
- Старые версии API будут поддерживаться минимум 6 месяцев после релиза новой версии
- Breaking changes будут происходить только в новых major версиях
- Минорные версии (v1.1, v1.2) будут добавлять новые функции без breaking changes
...
7. **Используйте версионированные URL** когда они станут доступны
8. **Проверяйте заголовок X-API-Version** для определения версии API
